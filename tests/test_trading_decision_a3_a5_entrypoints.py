...
def test_a3_entrypoint_builds_strategy_artifact(tmp_path: Path):
    ...
    assert out["stage_id"] == "A3"
    assert out["strategy_mode"] in {"trend_follow", "mean_revert", "neutral"}
...
def test_a4_entrypoint_outputs_risk_gate(tmp_path: Path):
    ...
    assert out["stage_id"] == "A4"
    assert out["risk_gate"] in {"PASS", "REVIEW", "BLOCK"}
    assert Path(out["artifact_path"]).exists()
...
def test_a5_entrypoint_outputs_execution_plan(tmp_path: Path):
    ...
    assert out["stage_id"] == "A5"
    assert out["order_plan"]["side"] in {"BUY", "SELL"}
    assert Path(out["artifact_path"]).exists()
